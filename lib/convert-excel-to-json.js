...
const excelToJson = (function() {
    ...
    const parseSheet = (sheetData, workbook) => {
        ...
        for (let cell in sheet) {
            ...
            rowData[columnData] = (sheet[cell].t == 'n') ? sheet[cell].w.trim() : (sheet[cell].v.trim && sheet[cell].v.trim()) || sheet[cell].v;
    ...
    const convertExcelToJson = function(config) {
        console.log('config', config.constructor === String, config);
        ...
        sheetsToGet.forEach(sheet => {
            sheet = (sheet.constructor == String) ? { name: sheet } : sheet;

            const { rows, header } = parseSheet(sheet, workbook);

            if(config.stubCellsBasedOnHeader && header && header.length){
                rows.forEach(row => {
